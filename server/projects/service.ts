...
const SUPABASE_PROJECT_SELECT = "id, name, status, template_id, dimensions, faces, workspace, created_at, updated_at";
const SUPABASE_LEGACY_PROJECT_SELECT = "id, name, status, dimensions, faces, workspace, created_at, updated_at";
...
async function writeFileProjectSnapshot(id: string, input: ProjectInput, existing?: Project): Promise<Project> {
  ...
  const project: Project = {
    ...
    templateId: normalizeTemplateId(input.templateId, existing?.templateId),
...
async function readFileProject(id: string): Promise<Project | null> {
  ...
  if (dbProject) {
    return normalizeProjectRecord(dbProject, id);
  }
...
async function listSupabaseProjects(options: ProjectListOptions): Promise<Project[]> {
  ...
  let { data, error } = await request.returns<ProjectRow[]>();

  if (error && isMissingTemplateIdError(error)) {
    let legacyRequest = supabase
      .from(SUPABASE_TABLE)
      .select(SUPABASE_LEGACY_PROJECT_SELECT)
      .order("updated_at", { ascending: false })
      .limit(limit);

    if (query) {
      legacyRequest = legacyRequest.ilike("name", `%${query}%`);
    }

    if (status) {
      legacyRequest = legacyRequest.eq("status", status);
    }

    const legacyResult = await legacyRequest.returns<ProjectRow[]>();
...
async function writeSupabaseProject(project: Project): Promise<void> {
  ...
  let { error } = await supabase.from(SUPABASE_TABLE).upsert(row, { onConflict: "id" });
  ...
  if (error && isMissingTemplateIdError(error)) {
    ...
    const legacyResult = await supabase.from(SUPABASE_TABLE).upsert(legacyRow, { onConflict: "id" });
    error = legacyResult.error;
  }
...
function normalizeProjectRecord(value: unknown, fallbackId?: string): Project | null {
  ...
  return {
    ...
    templateId: normalizeTemplateId(candidate.templateId),
...
function isMissingTemplateIdError(error: unknown): boolean {
  const candidate = error as { message?: string };
  return isMissingSupabaseTableOrColumnError(error) && candidate.message?.toLowerCase().includes("template_id") === true;
}
